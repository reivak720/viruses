...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    x_vals = pylab.zeros([300])

    for _ in range(numTrials):
        viruses = [SimpleVirus(maxBirthProb, clearProb) for _ in range(numViruses)]
        patient = Patient(viruses, maxPop)
        iterator = (patient.update() for _ in range (300))
        x_vals += pylab.fromiter(iterator, dtype = float)        
    pylab.plot(x_vals/numTrials, label = "SimpleVirus")
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        """
        prob = self.maxBirthProb *(1-popDensity)
        if all(self.isResistantTo(drug) for drug in activeDrugs) and random.random() <= prob:
                inhertProb = 1-self.getMutProb()
                new_resistances = {key:value if random.random()<= inhertProb else not value for key, value in self.getResistances().items()}
...
def simulationWithDruga(numViruses, maxPop, maxBirthProb, clearProb, resistances,
                       mutProb, numTrials):
    """
    ...
    for _ in range(numTrials):
        ...
        result1 = [[patient.update(), patient.getResistPop(['guttagonol'])] for _ in range(150)]
...
if __name__ == "__main__":
    random.seed(0)
    #simulationWithoutDrug(100, 1000, .1, 0.05, 1)
    #simulationWithoutDrug(1, 10, 1.0, 0.0,10)
  #  simulationWithoutDrug(100, 1000, .1, 0.05, 10)
  #  simulationWithoutDrug(1, 10, 1.0, 0.0,10)
   # simulationWithDruga(1, 10, 1.0, 0.0, {}, 1.0, 5)
    simulationWithDruga(100, 1000, 0.1, 0.05,  {'guttagonol': False}, 0.005, 20)
